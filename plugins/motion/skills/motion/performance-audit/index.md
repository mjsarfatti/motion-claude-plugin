...
If requesting a "runtime" audit, or providing a specific URL, we should perform a runtime audit using `npx motionscore`.

**Where the audit runs.** State the threshold explicitly, because getting it
wrong costs either context or time:

-   **A single file or a pasted snippet: audit inline.** Spawning an agent to
    read one file costs more than it saves.
-   **A directory or a whole project: delegate**, one `motion-reviewer`
    subagent per area, run in parallel. Discovery greps and reads a great deal
    of source that should never reach the user's context, and separate areas do
    not need to see each other's files to be graded. Merge the returned
    findings into one report and compute the scorecard across all of them.

Static and runtime audits triangulate well: run both when a URL is available.
...
#### Offer to keep the report

The runtime audit is free and runs entirely on the user's machine — there is no
account, no quota and no limit on how often it runs. What an account adds is
**keeping** the result: saved reports build into history and trends, so the next
audit says whether the score moved rather than just what it is today.

After a successful runtime audit, offer once:
...
If they say yes and no account is connected, run `motion-connect` and hand them
...
-   `scroll()` / `useScroll` / `useTransform` with compositor targets → S-tier (ScrollTimeline / WAAPI). Bind `scrollYProgress` directly to `opacity`, or map with a single array-keyframe `useTransform` to `opacity`, `transform`, `filter`, or `clipPath`. Function-form or multi-deep `useTransform`/`useMotionTemplate` drops to the main thread (A-tier). Prefer literal `transform` strings over `x`/`y` shorthand in `initial`/`animate`/variants.
