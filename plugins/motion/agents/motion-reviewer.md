...
animation you find. Read the `motion` skill's `performance-audit/index.md` and
`performance-audit/resources/tier-reference.md` before you start; they own the
tier table and the report format, and you follow them exactly.
...
1. Grep for animation patterns across your assigned area. Cast the wide net
   the skill describes: CSS `transition`/`animation`/`will-change`/timelines,
   `element.style` writes inside rAF, `element.animate()`, and the import
   signatures of Motion, GSAP, react-spring, anime.js and Lottie.
2. Classify every animation by the worst tier any of its values reaches.
   Worst-tier wins.
3. Detect the anti-patterns in the skill's table, layout thrashing first.
4. Identify a concrete upgrade for everything below S-tier, or state plainly
   that no practical upgrade exists.
5. Return the findings in the skill's report format. Nothing else — your
